...
export const ContentList = styled.ul<themeMenuProps>`
    ${({ theme, themeMenu }) =>
        css`
            display: flex;

            @media (max-width: 1000px) {
                background-color: ${themeMenu
                    ? theme.colors.grayNav
                    : "#ebebeb"};
...
export const ImageHamb = styled(Image)<themeHambProps>`
    ${({ themehamb }) => css`
        display: none;
        filter: ${themehamb == "true" ? "" : "invert()"};
        &:hover {
            cursor: pointer;
        }
        @media (max-width: 1000px) {
